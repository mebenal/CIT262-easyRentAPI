...
async function createReservation() {
  ...
  try {
    ...
    funcResp.response = funcResp.response.substr(9); // Cuts off beginning of message, leaving just the reservation id.
  } catch (exception) {
...
it(`Testing to see if ${reservationUrl} is up`, async () =>{
  ...
  let str = `Create reservation is currently ${createResponse.errorWasCaught ? 'NOT ' : ''}working.\n` 
